...
13. [Current Capabilities, Limitations, and Open TODOs](#current-capabilities-limitations-and-open-todos)
...
This is a comprehensive interpretability experiment stack that combines:
...
The current codebase supports a broad Olmo-3 variant set (base, instruct, instruct_sft, instruct_dpo, think, think_sft, think_dpo, rl_zero) and multiple pressure families: legacy conditions (`control`, `asch_history_5`, `authoritative_bias`) plus Zhu-style variants (`asch_peer_pressure`, `authority_claim`) with tone/consensus/QD/DA controls.
...
6. Analysis and Reporting
   ...
   ↓
7. Posthoc / Backfill Utilities (optional)
   ├─ Backfill missing trial outputs
   ├─ Re-run judge scoring for existing runs
   ├─ Add answer-logprob tables
   ├─ Add contrastive steering + activation patching
   └─ Refresh run reports
```
...
Every experiment starts with a JSON configuration file. Commonly used configs in the current repo are:
- `experiments/olmo_conformity/configs/suite_expanded_temp0.0.json`
- `experiments/olmo_conformity/configs/suite_expanded_temp1.0.json`
- `experiments/olmo_conformity/configs/test_scripts/suite_expanded_localtest_by_model.json`

### Basic Structure

```json
{
  "suite_name": "olmo_conformity_expanded_temp0.0",
  "datasets": [
    {
...
This snippet is intentionally minimal. Real "expanded" suites add more datasets and condition variants (tone, consensus diversity, question distillation, and devil's-advocate mitigation).
...
- **synthetic_asch_history** (`asch_history_5`): Five confederates claim the wrong answer
- **authoritative_bias**: A confident user claim with wrong answer
- **asch_peer_pressure**: Zhu-style multi-participant pressure with configurable tone/consensus/distillation/devil's-advocate
- **authority_claim**: Zhu-style authority framing with optional devil's-advocate alternate

**Models**: Which AI models to test. Expanded suites currently include:
- **base**, **instruct**, **instruct_sft**, **instruct_dpo**
- **think**, **think_sft**, **think_dpo** (think-tag capable variants)
- **rl_zero** (math-specialized RL variant)
...
**Configuration**: `suite_expanded_temp0.0.json`
...
4. **Probe Training** (if run):
   - Truth probe trained on `datasets/candidates/truth_probe_train.jsonl`
   - Social probe trained on `datasets/candidates/social_probe_train.jsonl`
...
**Configuration**: `suite_expanded_temp1.0.json`
...
## Current Capabilities, Limitations, and Open TODOs {#current-capabilities-limitations-and-open-todos}

### What is currently implemented in code

- Unified CLI under `python -m vivarium` with experiment subcommands such as `olmo-conformity`, `olmo-conformity-full`, `olmo-conformity-posthoc`, `olmo-conformity-judgeval`, `olmo-conformity-logit-lens`, `olmo-conformity-intervene`, `olmo-conformity-backfill-behavioral`, and `olmo-conformity-complete-suite`.
- Behavioral trial engine supports both legacy pressure prompts and Zhu-style prompt families with deterministic prompt rendering metadata (`PROMPT_RENDERER_VERSION`) persisted to DB.
- Scientific guardrails enforce wrong-answer presence and wrong-vs-ground-truth mismatch for pressure conditions (`_get_wrong_answer`), preventing answer-leakage trials.
- Per-variant probe training and variant-scoped intervention filtering are implemented in orchestration, with run artifacts under `runs/<timestamp>_<run_id>/`.
- Posthoc pipeline can add answer logprobs, logit lens tug-of-war, contrastive steering, activation patching, interventions, and refreshed figures/tables.

### Practical limitations in the current code

- Several interpretability paths require optional extras (`torch`, `safetensors`, `transformer-lens`, plotting libs). Missing dependencies cause skip/fail behavior depending on command.
- Activation capture is not available when using remote API inference (`--api-base`) for Olmo models; capture requires local hooked/TransformerLens-compatible execution.
- Some analytics are best-effort: `analysis.generate_core_figures()` skips outputs when prerequisites/tables are missing.
- Judge-based scoring can leave parse-error rows unscored; these require re-run (`--retry-parse-errors`) and are intentionally not force-filled.

### Explicit open TODO still in repository

- `src/vivarium/analytics/activations.py` includes an unfinished path in `generate_activation_embeddings()`: loading tensor payloads for embedding/PCA/UMAP is still placeholder-level (`"Activation tensor loading not yet implemented"`). Coverage/statistics and condition-wise comparisons are implemented, but that embedding path is not complete.
...
- **`src/vivarium/run.py`**: Main entry point and top-level CLI parsing
- **`src/vivarium/experiments/olmo_conformity/cli.py`**: Experiment command registration and handlers
- **`src/vivarium/experiments/olmo_conformity/runner.py`**: Behavioral trial execution and prompt rendering
- **`src/vivarium/experiments/olmo_conformity/orchestration.py`**: Full workflow coordination
- **`src/vivarium/experiments/olmo_conformity/probes.py`**: Probe capture/training/projections
- **`src/vivarium/experiments/olmo_conformity/intervention.py`**: Vector subtraction interventions
- **`src/vivarium/experiments/olmo_conformity/analysis.py`**: Figure and table generation
- **`src/vivarium/experiments/olmo_conformity/vector_analysis.py`**: Truth vs. social vector analysis
- **`src/vivarium/experiments/olmo_conformity/logit_lens.py`**: Logit lens and think-token analyses
- **`src/vivarium/experiments/olmo_conformity/contrastive_steering.py`**: Contrastive steering analyses
- **`src/vivarium/experiments/olmo_conformity/activation_patching.py`**: Activation patching analyses
- **`src/vivarium/persistence.py`**: Database schema and operations
- **`src/vivarium/interpretability.py`**: Activation capture infrastructure
- **`src/vivarium/llm_gateway.py`**: Model interface layer (HF-hooked, LiteLLM, TransformerLens, mock)

### Configuration Files

- **`experiments/olmo_conformity/configs/suite_expanded_temp0.0.json`**: Expanded suite at temperature 0.0
- **`experiments/olmo_conformity/configs/suite_expanded_temp1.0.json`**: Expanded suite at temperature 1.0
- **`experiments/olmo_conformity/configs/test_scripts/suite_expanded_localtest_by_model.json`**: Model-first local test suite
...
- **`experiments/olmo_conformity/datasets/candidates/truth_probe_train.jsonl`**: Truth probe training candidates
- **`experiments/olmo_conformity/datasets/candidates/social_probe_train.jsonl`**: Social probe training candidates
