...
from vivarium.channel import Channel
from vivarium.persistence import TraceDb
from vivarium.policy import AgentPolicy
from vivarium.types import ActionRequest, ActionResult, Observation, TraceEvent
from vivarium.tools import default_tools

if TYPE_CHECKING:  # pragma: no cover
    from vivarium.agent_state import AgentStateSpace
    from vivarium.interpretability import CaptureContext
    from vivarium.memory import MemoryManager
    from vivarium.domain_state import GenericDomainState
...
class WorldEngine:
    def __init__(
        ...
        *,
        ...
        agent_state_space: Optional["AgentStateSpace"] = None,
    ):
        self._config = config
        ...
        self._agent_state_space = agent_state_space
    ...
    def build_observation(self, *, time_step: int, agent_id: str) -> Observation:
        ...
        # Enrich with agent state if available (AgentStateSpace extension)
        if self._agent_state_space is not None:
            state = self._agent_states.get(agent_id)
            if state is None:
                state = self._agent_state_space.init_state(agent_id)
                self._agent_states[agent_id] = state
            state_obs = self._agent_state_space.observe(agent_id=agent_id, state=state, time_step=time_step)
            if state_obs:
                obs["agent_state"] = state_obs
    ...
    def commit_requests(self, *, time_step: int, reqs: List[ActionRequest]) -> None:
        ...
        for idx, req in enumerate(reqs):
            ...
            if self._agent_state_space is not None:
                ...
                if current is None:
                    current = self._agent_state_space.init_state(req.agent_id)
                next_state = self._agent_state_space.transition(
                    agent_id=req.agent_id,
                ...
                self._agent_states[req.agent_id] = next_state
                self._trace_db.insert_agent_state(
                    run_id=self._config.run_id,
                    time_step=time_step,
                    agent_id=req.agent_id,
                    state_json=json.dumps(next_state, sort_keys=True, separators=(",", ":"), ensure_ascii=False),
                    created_at=ts,
                )
