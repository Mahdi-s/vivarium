...
Environment Variables:
    VIVARIUM_MODEL_DIR: Directory for storing large model weights (GGUF, Safetensors)
    VIVARIUM_MODELS_DIR: Alias for VIVARIUM_MODEL_DIR (back-compat)
    VIVARIUM_LLAMA_CPP_ROOT: Path to llama.cpp repository
    VIVARIUM_ARTIFACTS_DIR: Root directory for simulation outputs (.db, activations)
    VIVARIUM_RUNS_DIR: Alias for VIVARIUM_ARTIFACTS_DIR (back-compat)
    VIVARIUM_HF_CACHE: HuggingFace downloader cache location

    Legacy (back-compat): AAM_MODEL_DIR, AAM_MODELS_DIR, AAM_LLAMA_CPP_ROOT,
    AAM_ARTIFACTS_DIR, AAM_RUNS_DIR, AAM_HF_CACHE
...
class VivariumSettings:
    ...
    def MODEL_DIR(self) -> Path:
        """Directory for model weights. Override with VIVARIUM_MODEL_DIR env var."""
        env_val = (
            os.environ.get("VIVARIUM_MODEL_DIR")
            or os.environ.get("VIVARIUM_MODELS_DIR")
            or os.environ.get("AAM_MODEL_DIR")
            or os.environ.get("AAM_MODELS_DIR")
        )
        if env_val:
            return Path(env_val)
    ...
    def LLAMA_CPP_ROOT(self) -> Path:
        """
        Path to llama.cpp repository. Override with VIVARIUM_LLAMA_CPP_ROOT env var.
        
        Falls back to:
        1. VIVARIUM_LLAMA_CPP_ROOT or AAM_LLAMA_CPP_ROOT environment variable
    ...
    def ARTIFACTS_DIR(self) -> Path:
        """Root directory for run artifacts. Override with VIVARIUM_ARTIFACTS_DIR env var."""
        env_val = (
    ...
    def HF_CACHE(self) -> Path:
        """HuggingFace cache directory. Override with VIVARIUM_HF_CACHE env var."""
    ...
    def get_llama_cpp_convert_script(self) -> Path:
        ...
        if not script.exists():
            raise FileNotFoundError(
                ...
                "or set VIVARIUM_LLAMA_CPP_ROOT environment variable."
            )
        return script
