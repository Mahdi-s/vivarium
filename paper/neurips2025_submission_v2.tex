...
\begin{abstract}
When a language model is shown a chat in which several fabricated peers all confidently endorse a wrong answer, it often goes along with the crowd, even on questions it would otherwise answer correctly. The recent alignment literature has described this as a form of social conformity in LLMs, framed in the tradition of Solomon Asch's classic experiments, and has reported that instruction tuning gradually reduces it. We revisit this picture and argue that the canonical Asch-style probe carries a structural-repetition confound that prior work has not isolated, and that the social framing accounts for less of the measured signal than has been assumed. To make this case we needed a model family open enough to inspect from the inside, and OLMo-3 is the first major release to publish every intermediate post-training checkpoint together with the training mixtures used at each stage. Working within that family, we do three things at once: we follow the model's behavior stage by stage along its post-training pipeline, audit the supervised-fine-tuning and preference-optimization corpora directly, and design a prompt ablation that strips out the social framing while leaving the repetitive surface form of the prompt intact.
\end{abstract}
...
Throughout this paper we follow OLMo-3 through three post-training stages: supervised fine-tuning (SFT), direct preference optimization (DPO;~\citealt{rafailov2023dpo}), and reinforcement learning from verifiable rewards (RLVR).
...
The behavioral results in \S\ref{sec:stage_decomp} and the cross-family analysis in Appendix~\ref{app:cross_family} were collected first; the corpus audit in \S\ref{sec:pillar1}--\S\ref{sec:pillar2} was designed \emph{after} the behavioral results were known. Pre-registration discipline was applied \emph{within} the audit pipeline but does \emph{not} protect the broader paper from confirmation bias. We therefore describe Pillars I and II as corpus-level observations \emph{consistent with} the behavioral trajectory, not as causal mechanism.
...
The same shape holds across the broader condition set: across every peer-consensus condition, Instruct-SFT exceeds Base, Instruct-DPO falls below Instruct-SFT, and the final Instruct (RLVR) sits in between. The Think pipeline shows the same \emph{shape} at a lower absolute level. Two stages with putatively different objectives (SFT and DPO) move the behavior in opposite directions; the final released checkpoint is the algebraic sum of those two opposing pulls. The full BER heatmap across all 12 conditions and all checkpoints is in Figure~\ref{fig:heatmap} (Appendix~\ref{app:heatmap}).
...
\begin{wrapfigure}{r}{0.50\linewidth}
  ...
  \centering
  \includegraphics[width=\linewidth]{fig_pillar_1_carryover_bars.pdf}
  \caption{\textbf{Pillar~I: Structural priors are corpus-localized.}
  Top-12 Dolci source datasets by list-carryover rate
  $P(\text{resp\_has\_list}\mid\text{prompt\_has\_list})$.
  Three Tulu/Math sources exceed $0.80$; corpus-wide mean is $0.461$.
  Per-source affirmation-prefix vs.\ max-run-$\geq 5$ scatter is in
  Appendix~\ref{app:pillar1_scatter}.}
...
\noindent The Dolci-Instruct-DPO contrast is the natural complement to Pillar~I. If SFT rewards prompt-structure mirroring, what does DPO push against? We compute Cliff's $\delta$~\citep{cliff1993dominance} between the rejected and chosen distributions on nine pre-registered structural metrics over the full corpus ($N{=}259{,}785$ chosen/rejected pairs), with paired-bootstrap 95\% CIs and a sign-flip permutation null distribution at $1{,}000$ reps (Figure~\ref{fig:pillar2_effect}; full numerical summary in Table~\ref{tab:data_audit}, Appendix~\ref{app:pillar2_table}).

\begin{wrapfigure}{r}{0.50\linewidth}
  \vspace{-0.5\baselineskip}
  \centering
  \includegraphics[width=\linewidth]{fig_pillar_2_forest_plot.pdf}
  \caption{\textbf{Pillar~II: DPO penalizes content copying but not literal repetition runs.}
  Effect-size forest plot over all nine delta metrics ($N{=}259{,}785$ pairs),
  with 95\% bootstrap CIs and Holm-corrected markers (filled blue:
  $p_{\text{Holm}}{<}0.05$). Null-distribution $z$ scores annotate each row.
  The Romano et al.~(2006) ``none'' band ($|\delta|{<}0.147$, grey) is shown for
  reference; directional sign is the more honest readout at this $N$.
  Violin distributions for four representative metrics are in
  Appendix~\ref{app:pillar2_violins}.}
...
% [Table~\ref{tab:data_audit} moved to Appendix~\ref{app:pillar2_table}]
...
All standard conditions use a system prompt that explicitly warns against the conformity trap: ``You are a truthful assistant. You may see prior users' answers. Prefer correctness over agreement, even if a prior group consensus exists.'' Removing this prompt while keeping the Asch-style pressure identical, the \emph{naked} condition, reveals a model-dependent instruction-hierarchy effect~\citep{wallace2024instruction}: for some models endorsement rises sharply without the system prompt, while for others the system prompt provides no measurable protection at all. On Llama-3.1-70B the system-prompt-protected BER is $4.5\%$ but coexists with an $89\%$ refusal rate, so the protection takes the form of refusal-based abstention, not corrected answering; on models without that hierarchy-driven refusal protection, the system prompt provides no measurable effect.

\paragraph{Ablation~2: Stripping social framing while keeping repetition.}
Replacing the Asch-style prompt with a non-social n-gram baseline, identical repetition structure, no participant roles, no organizer, no group framing, yields wrong-answer endorsement \emph{at least as high} as the naked social prompt for both tested models. On Llama-3.1-70B the original-wording n-gram BER is $0.365$ vs $0.278$ under the naked social prompt ($T{=}0.0$); on OLMo-3.1-32B-Instruct it is $0.455$ vs $0.390$ (Figure~\ref{fig:ablation}, Appendix~\ref{app:ablation_figure}). Removing the social framing does not reduce the effect on these two models; on the n-gram baseline it sits somewhat higher---but as the next paragraph shows, part of that gap is attributable to an instruction-wording difference rather than to the social framing itself.

\paragraph{An instruction-wording confound, and the matched-instruction follow-up.}
The n-gram baseline uses ``Answer the following question \emph{based on the provided sequence},'' while the social prompt reads ``Answer the question \emph{based on your knowledge}'' (Appendix~\ref{app:prompts}). The two prompts therefore differ in instruction framing as well as social context. A matched-instruction follow-up (n-gram prompt, same instruction stem as the social condition, no participant roles) completes the decomposition. On Llama-3.1-70B ($T{=}0.0$, $N{=}400$) the result is a three-level hierarchy: the original n-gram (``adhere to sequence'') yields BER~$=0.365$; the naked social prompt and the matched-instruction n-gram both yield BER~$=0.278$. The ${\approx}9$-point gap between the two n-gram variants is an instruction-wording effect~\citep{wallace2024instruction}; once that is controlled, the structural and social channels are statistically indistinguishable on this model. The OLMo-3.1-32B-Instruct matched-instruction condition is pending; full details and figures are in Appendix~\ref{app:ablation_figure}.

\paragraph{What the ablation does and does not show.}
\emph{What is supported:} on the canonical 5-repetition Asch template, substantial wrong-answer endorsement is reproducible by autoregressive pattern completion~\citep{min2022rethinking, shi2023large} on the repetitive surface form alone, even after removing the instruction-wording confound. The explicit ``adhere to sequence'' instruction adds a further ${\approx}9$-point increment, consistent with decoders' known sensitivity to in-context repetition~\citep{holtzman2020curious, welleck2020unlikelihood}. \emph{What is not supported:} the social channel is not eliminated. Authority-bias conditions (one social claim, no repetition) produce significant endorsement (Appendix~\ref{app:mcnemar}); for some models it is the \emph{stronger} pressure (Llama-4-Maverick, OR=$20.00$). On Llama-3.1-70B the two channels are roughly equal in magnitude. The right reading is that the canonical probe conflates a structural repetition channel, a social framing channel, and an instruction-wording effect in model-dependent proportions. Devil's-advocate prompting~\citep{zhu2025conformity} reduces but does not eliminate the effect (Appendix~\ref{app:mitigation}), consistent with both readings.

% [Ablation figure moved to Appendix~\ref{app:ablation_figure} to save space]

The relationship between the ablation results and the corpus measurements is discussed in \S\ref{sec:discussion}.
...
The four results in \S\ref{sec:results} are mutually consistent with a structural reading, and the matched-instruction ablation sharpens the claim: a substantial portion of the measured signal is reproducible by autoregressive pattern completion alone, even after removing the instruction-wording confound. We do \emph{not} argue that LLM social deference is illusory: non-repetitive social cues (authority-bias) also produce significant endorsement (Appendix~\ref{app:mcnemar}, Appendix~\ref{app:cross_family}), so a social channel exists and its weight is template- and model-dependent. The right summary is that the canonical probe conflates multiple components in proportions the field has not measured.

\paragraph{What this implies for mitigation.}
Mitigations that target only the social framing will leave the structural component untouched, and vice versa. Warnings against social pressure are useful for some models; mitigations that disrupt the prompt's repetitive surface form---reformatting peer claims as free text, randomizing position, or interleaving---are likely to address the structural component that warnings alone do not reach (see Appendix~\ref{app:mitigation}).

\paragraph{What about the other model families?}
Appendix~\ref{app:cross_family} extends the evaluation to eight additional model families. The cross-family ranking partitions into three qualitatively distinct response modes, endorsement-dominant, refusal-dominant, and context-insensitive, which a pooled error rate would conflate (Figure~\ref{fig:taxonomy}), and the OLMo training-stage checkpoints sit in the cross-sectional order \S\ref{sec:stage_decomp} predicts. Reasoning models cluster at the bottom of the BER ranking; under the structural reading, the initial \texttt{<think>} token mechanically interrupts the n-gram chain the fabricated peers establish, which is exactly what the pattern-completion account predicts (Appendix~\ref{app:reasoning_caveat}).

\paragraph{Limitations.}
\label{sec:limitations}
Full limitations are enumerated in Appendix~\ref{app:limitations}; items (3), (6), and (7) are most structurally consequential.
...
We came to this paper expecting to measure social conformity in language models, and we leave with a more careful version of the question. Within the OLMo-3 pipeline, the post-training stages pull in opposite directions: SFT amplifies wrong-answer endorsement under in-context pressure; DPO partially reverses it. The released training mixtures show, in a corpus-level pattern consistent with that trajectory, that SFT rewarded prompt-structure mirroring at high rates in math and science demonstrations and that DPO penalized prompt-content copying but not literal repetition runs. And when we strip the social framing while keeping the repetitive surface intact, the measured ``conformity'' rate does not collapse---it equals the social channel in magnitude once instruction wording is held constant (\S\ref{sec:ablation}).

The canonical 5-repetition probe conflates a structural channel (autoregressive pattern completion on a repetitive surface form) with a social channel (deference to a fabricated consensus), in proportions that vary by model and template. A substantial portion of what the literature has called LLM conformity is reproducible without any social cue; non-repetitive social cues produce smaller but real effects. Both channels exist; the field's measurement instrument has not separated them. What we offer is a sharper specification of what the vulnerability is, what the corpora encode, and what a clean measurement of LLM social deference will require. Code and materials will be released upon publication.
...
%% ============================================================
\section{BER Heatmap: Full Condition Set}
\label{app:heatmap}
...
%% ============================================================
\section{Ablation Figure: N-gram vs.\ Social Conditions}
\label{app:ablation_figure}

\begin{figure}[H]
  \centering
  \includegraphics[width=\linewidth]{fig3_ablation_ngram.pdf}
  \caption{\textbf{Four-condition ablation: Llama-3.1-70B ($T{=}0.0$, $N{=}400$).}
  BER under four conditions, left to right.
  (1)~\textbf{Social + system prompt} (blue): $4.5\%$, but $89\%$ refusal rate---protection
  is refusal-based abstention, not corrected answering.
  (2)~\textbf{Naked social} (orange): $27.8\%$; social Asch-style pressure without the
  system prompt, instruction reads ``Answer the question based on your knowledge.''
  (3)~\textbf{N-gram, matched instruction} (green): $27.8\%$; identical
  repetition structure, no social framing, same instruction stem as (2).  Social framing
  accounts for zero additional endorsement once instruction wording is held constant.
  (4)~\textbf{N-gram, original wording} (red): $36.5\%$; same structure but instruction
  reads ``Answer the following question based on the provided sequence,'' explicitly directing
  the model to the in-context sequence.
  The ${\approx}9$-point gap between (3) and (4) is an instruction-wording effect
  (\S\ref{sec:ablation}), not a social one.
  Error bars show Wilson 95\% CIs.
  OLMo-3.1-32B-Instruct matched-instruction condition is pending (see body text).}
  \label{fig:ablation}
\end{figure}

%% ============================================================
\section{Pillar~I Supplement: Affirmation-Prefix vs.\ Run-Length Scatter}
\label{app:pillar1_scatter}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.72\linewidth]{fig_pillar_1_affirm_vs_runlength.pdf}
  \caption{\textbf{Pillar~I supplement: two structural priors partially dissociate.}
  Per-source affirmation-prefix rate (x-axis) vs.\ max-run-$\geq 5$ rate (y-axis) across all
  Dolci-Instruct-SFT sources. The two priors co-occur but with weak rank correlation
  ($\rho{=}0.15$), indicating they identify partially distinct surface-mirroring failure modes
  in the corpus. Sources appearing in the top-12 of Figure~\ref{fig:pillar1_priors} are
  highlighted.}
  \label{fig:pillar1_scatter}
\end{figure}

%% ============================================================
\section{Pillar~II Supplement: Violin Distributions}
\label{app:pillar2_violins}

\begin{figure}[H]
  \centering
  \includegraphics[width=0.70\linewidth]{fig_pillar_2_violins.pdf}
  \caption{\textbf{Pillar~II supplement: violin distributions for four representative delta metrics.}
  $N{=}50{,}000$ Dolci-Instruct-DPO pairs (subsample, seed $0$, clipped to $[-0.5, +0.5]$ for
  display). Each violin shows the full empirical distribution; the white box is the IQR;
  Cliff's $\delta$ and practical significance are annotated above each violin.
  The effect-size forest plot over all nine metrics is Figure~\ref{fig:pillar2_effect}
  in the main text.}
  \label{fig:pillar2_violin_app}
\end{figure}

%% ============================================================
\section{Pillar~II Effect-Size Table}
\label{app:pillar2_table}
