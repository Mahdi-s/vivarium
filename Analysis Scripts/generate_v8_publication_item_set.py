#!/usr/bin/env python3
"""
V8 Publication Item-Set Generator with Statistical Analysis.

Extends V7 with:
  - Full 12-condition judge-label coverage for item-set gating
  - Trial-level data loading with judge-JSON parsing
  - Balance validation across (temperature, variant, condition) cells
  - Inferential statistical tests:
      * McNemar test per (variant, pressure_condition) vs control
        with Holm-Bonferroni correction, odds ratios, Cohen's h
      * Cochran's Q test across condition families (tone, authority, peer)
      * BCa bootstrap 95% CIs on pooled truth-override and delta-error
  - Publication-quality figures:
      A. Forest plot: pressure effects with 95% CIs + significance
      B. Heatmap: variant × condition with significance stars
      C. Tone modulation: plain/neutral/confident/uncertain comparison
      D. Authority variant comparison: authoritative_bias / trust / trust+DA
      E. Temperature CI bands: truth override vs T for core + sub-conditions
      F. Mitigation effectiveness: DA/AD/diverse vs unanimous plain

Primary presentation (core conditions):
  control  /  asch_history_5  /  authoritative_bias

Sub-condition analysis covers all 12 conditions grouped into families.
All display labels strip the "zhu" substring.
...
Usage:
  python 'Analysis Scripts/generate_v8_publication_item_set.py' \\
      ...
      --out-dir Comparing_Experiments/v8_publication \\
      --run-analysis

Add --n-boot N to change the number of bootstrap resamples (default 10000).
"""
...
# All 12 conditions (order matters: control first, then peer family, then authority)
...
# Family tags used for coloring and grouping
...
EXCLUDED_VARIANTS: Tuple[str, ...] = ("rl_zero", "think_dpo")

VARIANT_ORDER: List[str] = [
    "base", "instruct", "instruct_sft", "instruct_dpo", "think", "think_sft",
]
...
# ---------------------------------------------------------------------------
# Phase 1: Item-set generation (extended from V7)
# ---------------------------------------------------------------------------
...
def query_run_judge_coverage(
    ...
    """
    For a single run, return coverage statistics and the set of fully-covered items.

    An item is "fully covered" when every included model has a valid
    parsed_answer_json label for every judge_condition.
    """
...
def _parse_judge_json(pj: Any) -> Optional[Dict[str, Any]]:
    """Parse and validate judge JSON from parsed_answer_json. Returns None if invalid."""
    if pj is None:
        return None
...
def load_trial_data_for_run(
    ...
    Only returns rows with valid parsed judge JSON.
    """
    conn = sqlite3.connect(str(db_path))
...
def check_balance(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns a DataFrame flagging cells that deviate >5% from the median cell size.
    """
...
def _mcnemar_exact_p(b: int, c: int) -> float:
    """Two-tailed exact McNemar p-value via binomial CDF (scipy) with chi-squared fallback."""
    total = b + c
    if total == 0:
        return 1.0
    try:
...
def compute_mcnemar_tests(df: pd.DataFrame) -> pd.DataFrame:
    """
    McNemar test per (variant, pressure_condition) vs control.
    Pools all (item, temperature) matched pairs across factual datasets.

    Columns: variant, condition_name, condition_label, condition_family,
             n_pairs, b, c, error_rate_control, error_rate_pressure, delta_error,
             odds_ratio, cohens_h, p_raw, p_adjusted, sig_raw, sig_adjusted
    """
    factual = df[df["is_factual"] & df["is_correct"].notna()].copy()
    if factual.empty:
        return pd.DataFrame()
    ...
    for cond in pressure_conds:
        ...
        for variant in sorted(merged["variant"].unique()):
            ...
            b_c, c_c = b + 0.5, c + 0.5  # continuity correction for OR
            odds_ratio = b_c / c_c
            rows.append({
                "variant": variant,
                "condition_name": cond,
...
def _cochrans_q_stat(X: np.ndarray) -> Tuple[float, float]:
    """
    Cochran's Q for an (n_subjects × k_conditions) binary matrix.
    Returns (Q_statistic, p_value) using chi-squared(k-1) approximation.
    """
...
def compute_cochrans_q_families(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Uses wrong-answer endorsement (agrees_wrong_answer) as the binary outcome.
    Families tested: tone (4), mitigation (3), authority (3), peer_full (all 8).
    """
    factual = df[df["is_factual"] & df["agrees_wrong_answer"].notna()].copy()
    if factual.empty:
        return pd.DataFrame()
...
def _bca_ci_from_item_arrays(
    ...
    """
    BCa bootstrap CI computed from per-item count arrays.

    Item-level resampling preserves the paired (item × temperature) structure.
    Uses efficient vectorised jackknife to avoid O(n²) leave-one-out loops.

    metric: "truth_override_rate" | "delta_error"
    """
    ...
    # Vectorised jackknife acceleration
    if metric == "truth_override_rate":
...
def compute_bootstrap_cis(
    ...
    Resamples at the item level (pooled across temperatures), preserving
    the paired design. Per-item count arrays enable vectorised jackknife.
    """
    factual = df[df["is_factual"] & df["is_correct"].notna()].copy()
    if factual.empty:
        return pd.DataFrame()
...
def _mpl():
    """Import matplotlib.pyplot with Agg backend."""
    import matplotlib
    matplotlib.use("Agg")
...
def plot_forest_pressure_effects(
    ...
    """
    Forest plot: delta-error for every pressure condition, faceted by variant.
    Points filled when McNemar significant (Holm-Bonferroni adjusted).
    Core conditions (asch_history_5, authoritative_bias) shown as diamonds.
    """
    mpl = _mpl()
    import matplotlib.pyplot as plt
    ...
    # Find separator y between peer and authority families
    peer_last = next(
    ...
    for ax, variant in zip(axes, variants_present):
        ...
        ax.set_xlabel("Δ Error Rate vs Control", fontsize=8)
...
def plot_heatmap_with_stars(
    ...
    """
    Heatmap: truth-override rate (variant × condition).
    Cells annotated with point estimate and McNemar significance stars.
    Peer and authority families separated by a vertical line.
    """
    ...
    ax.set_title(
        ...
        "(* p<0.05  ** p<0.01  *** p<0.001 after Holm-Bonferroni; core conditions bold-bordered)",
        fontsize=10, pad=14,
    )
    # Bold frame around core conditions
    for c_i, cond in enumerate(present_conds):
...
def plot_tone_comparison(
    ...
    """
    Tone modulation: wrong-answer agreement across plain/neutral/confident/uncertain.
    Grouped bars per variant; Wilson 95% CI error bars; Cochran's Q annotation.
    """
    import numpy as np
    mpl = _mpl()
    import matplotlib.pyplot as plt
...
def plot_authority_comparison(
    ...
    """
    Authority variant comparison: authoritative_bias / trust / trust+DA.
    Grouped bars per variant; Wilson 95% CI; Cochran's Q annotation.
    """
    import numpy as np
    mpl = _mpl()
    import matplotlib.pyplot as plt
...
def plot_temperature_ci_bands(
    ...
    """
    Truth override rate vs temperature with 95% Wilson CI bands.
    Faceted by variant; lines for core conditions + representative sub-conditions.
    """
    import numpy as np
    mpl = _mpl()
    import matplotlib.pyplot as plt
...
def plot_mitigation_effectiveness(
    ...
    """
    Mitigation effectiveness: DA, AD, Diverse Plain vs Unanimous Plain (baseline).
    Bar chart per variant with McNemar significance brackets (adjusted p).
    """
    import numpy as np
    mpl = _mpl()
    import matplotlib.pyplot as plt
    ...
    for ax, variant in zip(axes, variants_present):
        ...
        ax.set_ylabel("Wrong-Answer Agreement Rate" if ax is axes[0] else "", fontsize=9)
        ax.grid(axis="y", alpha=0.28, linestyle=":")
        # Highlight baseline bar with edge
        ax.patches[0].set_edgecolor("black")
        ax.patches[0].set_linewidth(2.0)

    fig.suptitle(
        "Mitigation Effectiveness: DA / AD / Diverse vs Unanimous Plain (Baseline)\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="V8 Publication Item-Set Generator with Statistical Analysis."
    )
    ...
    parser.add_argument("--metadata", type=str, required=True,
                        help="Path to runs_metadata JSON (v6).")
    parser.add_argument("--out-dir", type=str, default="Comparing_Experiments/v8_publication",
                        help="Output directory.")
    parser.add_argument(
        "--run-analysis", action="store_true",
        help="Run Phases 2-5: load trials, compute metrics, run statistical tests, generate figures.",
    )
    parser.add_argument(
        "--n-boot", type=int, default=10000,
        help="Bootstrap resamples for BCa CIs (default: 10000).",
    )
    ...
    print("=" * 72)
    print("V8 Publication Item-Set Generator")
    ...
    print("\nPer-dataset counts in V8 item set:")
    ...
    # Write judge_coverage_by_model.csv (header includes 'complete' column)
    ...
    # Enforce variant ordering for plots
    ...
    print("  (estimating ~2–5 minutes depending on system speed)")
    ...
    print("\n" + "=" * 72)
    print("DONE. V8 publication analysis complete.")
    ...
    print(f"  Run with --run-analysis to regenerate metrics, stats, and figures.")
    print("=" * 72)
    return 0
