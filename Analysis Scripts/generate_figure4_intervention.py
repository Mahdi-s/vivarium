...
def main() -> int:
    p = build_arg_parser(description="Generate intervention figures/tables (Figure 4) for a run.")
    args = p.parse_args()
    ...
    try:
        metrics = compute_intervention_metrics(db, ref.run_id, ref.run_dir)
        figures = generate_intervention_graphs(db, ref.run_id, ref.run_dir, metrics)
        export_intervention_logs(db, ref.run_id, ref.run_dir, metrics)
    finally:
        db.close()
