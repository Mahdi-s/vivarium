# Engineer Handoff: Vivarium / AAM — OLMo Conformity + Interpretability
...
- The repo is two things living together:
  1) **Vivarium / Abstract Agent Machine (AAM)**: a deterministic multi-agent simulation kernel (Phases 1–4) with trace logging, replay, optional LLM cognition, and optional activation capture.
...
- Core package: `src/aam/`
  - `run.py`: **CLI** entrypoint (`aam` / `vvm`).
...
- Experiment implementation: `src/aam/experiments/olmo_conformity/`
...
- `sqlite3` (stdlib): persistence (`src/aam/persistence.py`) and many scripts.
- `argparse` (stdlib): CLI (`src/aam/run.py`) and most scripts.
- `pydantic`: suite/config validation (`src/aam/experiment_config.py`, `src/aam/experiments/olmo_conformity/suite_config.py`).
- `numpy`, `pandas`, `matplotlib`, `scipy`: analytics + plots (`src/aam/analytics/*`, `src/aam/experiments/olmo_conformity/analysis.py`).

**Model execution / local inference**
- `torch`: required for local inference + activation handling (`src/aam/llm_gateway.py`, `src/aam/interpretability.py`, interventions, answer logprobs).
...
- `langgraph`: orchestration for agent “think” graphs (`src/aam/agent_langgraph.py`).
- `json-repair`: utility used by the cognitive layer (where installed).

**Judge / evaluation**
- `httpx`: Ollama judge client (`src/aam/experiments/olmo_conformity/ollama_judge.py`).
- `judgeval`: optional inline judge scoring during the runner (`src/aam/experiments/olmo_conformity/judgeval_scorers.py`).

**Memory system (not central to conformity)**
- `chromadb`, `sentence-transformers`: vector memory (`src/aam/memory.py`).

### 1.4 `src/aam/` quick reference (core modules)
...
- `src/aam/run.py`: CLI, wires subcommands to modules.
- `src/aam/persistence.py`: schemas + insert/upsert helpers; if you change “what we store”, you change it here.
- `src/aam/llm_gateway.py`: inference gateways (remote + local); also houses `select_local_gateway()`.
- `src/aam/interpretability.py`: activation capture, sparse capture controls, safetensors sharding, metadata indexing, Merkle provenance.
- `src/aam/output_parsing.py`: output normalization + “output quality” classifier (prompt leakage, degenerate repetition, etc.) used for metadata.
- `src/aam/provenance.py`: Merkle accumulator primitives used by capture.
- `src/aam/world_engine.py`, `src/aam/scheduler.py`: deterministic simulation kernel + barrier scheduler (Phases 1–4).
- `src/aam/agent_langgraph.py`: cognitive agent policy abstraction that calls gateways and parses tool calls / JSON actions.
- `src/aam/tools.py`: tool definitions (what agents can do in the simulation).
- `src/aam/llama_cpp.py`, `src/aam/model_discovery.py`: local GGUF discovery + llama.cpp server integration (not used in OLMo conformity, except as an alternative inference backend).
- `src/aam/export.py`: Parquet export utilities.
...
All data is persisted in a single SQLite DB (`simulation.db`) via `TraceDb` (`src/aam/persistence.py`):
...
- schema: `src/aam/experiments/olmo_conformity/suite_config.py`
- loader: `src/aam/experiments/olmo_conformity/io.py:load_suite_config()`
...
- `src/aam/experiments/olmo_conformity/runner.py:run_suite()`
...
File: `src/aam/experiments/olmo_conformity/runner.py`
...
File: `src/aam/experiments/olmo_conformity/probes.py:capture_probe_dataset_to_db()`
...
File: `src/aam/experiments/olmo_conformity/intervention.py:run_intervention_sweep()`
...
File: `src/aam/experiments/olmo_conformity/logit_lens.py`
...
File: `src/aam/experiments/olmo_conformity/answer_logprobs.py:compute_and_store_answer_logprobs_for_model()`
...
All model access is abstracted behind gateway classes in `src/aam/llm_gateway.py`.
...
Function: `src/aam/llm_gateway.py:select_local_gateway()`
...
File: `src/aam/interpretability.py`
...
- `src/aam/experiments/olmo_conformity/runner.py:_build_prompt_for_condition()`

Helper utilities live in:
- `src/aam/experiments/olmo_conformity/prompts.py`
...
2) **Ollama post-hoc judge** (recommended / paper-friendly)
   - `src/aam/experiments/olmo_conformity/ollama_judge.py`
...
Entry:
- `src/aam/experiments/olmo_conformity/analysis.py:generate_core_figures()`

It calls modules under `src/aam/analytics/` when available:
...
- `scripts/fix_correctness_evaluation.py`: legacy re-scoring script for older DBs (current scoring lives in `src/aam/experiments/olmo_conformity/scoring.py`).
...
Recommendation:
- treat `src/aam/run.py` as authoritative for flags; update README when you change CLI.
...
- purely DB-derived metrics → implement in `src/aam/analytics/`
- requires new model calls → implement as a post-hoc command in `src/aam/run.py` + a module under `src/aam/experiments/olmo_conformity/`
