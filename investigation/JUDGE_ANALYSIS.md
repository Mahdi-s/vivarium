...
**Self-preference bias note**: GPT-OSS-20B is itself one of the 11 evaluated models. The 78/22 split mitigates potential self-preference bias by ensuring the majority of labels are assigned without LLM involvement.
...
**Gate status**: AT_RISK --- cross-family runs cover only 4 conditions (not the full 12).

**Models** (10 models x 2 temperatures T=0.0, T=0.6, plus 2 ablation DBs):
...
2. **GPT-OSS-20B incomplete data**: At T=0.0, GPT-OSS-20B is only 14.5% complete (232/1,600 outputs). Retry attempts failed, making this model's results unreliable at that temperature.
