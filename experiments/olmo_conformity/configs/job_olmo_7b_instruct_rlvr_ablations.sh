...
#SBATCH --job-name=AAM_7B_RLVR_NGRAM

# ---------------------------------------------------------------------------
# OLMo-3 7B Instruct (RLVR) — n-gram ablation (3 conditions x 8 datasets x 50 items).
# Conditions: (i) naked Asch, (ii) original-wording n-gram, (iii) matched-instruction n-gram.
#
# Usage:
#   TEMPERATURE=0.0 sbatch job_olmo_7b_instruct_rlvr_ablations.sh
#   TEMPERATURE=0.6 sbatch job_olmo_7b_instruct_rlvr_ablations.sh
...
if [ "${TEMPERATURE}" = "0.6" ]; then
    SUITE="suite_olmo_7b_instruct_rlvr_ablations_temp0p6.json"
else
    SUITE="suite_olmo_7b_instruct_rlvr_ablations_temp0p0.json"
fi
...
echo "=== OLMo 7B Instruct (RLVR) n-gram ablation: suite=${SUITE}, temperature=${TEMPERATURE} ==="
...
echo "=== OLMo 7B Instruct (RLVR) n-gram ablation (T=${TEMPERATURE}) complete ==="
