...
#SBATCH --job-name=AAM_32B_TD_PB

# ---------------------------------------------------------------------------
# OLMo-3 32B Think-DPO — Panel B extension: 8 conditions missing from the
# original Think suite, on the same 400-item set as Panel A.
#
# Conditions (from suite_olmo_32b_think_dpo_panelB_extension_temp{0p0,0p6}.json):
#   asch_history_5
...
#
# Usage:
#   TEMPERATURE=0.0 sbatch job_olmo_32b_think_dpo_panelB.sh
#   TEMPERATURE=0.6 sbatch job_olmo_32b_think_dpo_panelB.sh
...
if [ "${TEMPERATURE}" = "0.6" ]; then
    SUITE="suite_olmo_32b_think_dpo_panelB_extension_temp0p6.json"
else
    SUITE="suite_olmo_32b_think_dpo_panelB_extension_temp0p0.json"
fi
...
echo "=== 32B Think-DPO Panel B extension: suite=${SUITE}, temperature=${TEMPERATURE} ==="
...
echo "=== 32B Think-DPO Panel B extension (T=${TEMPERATURE}) complete ==="
