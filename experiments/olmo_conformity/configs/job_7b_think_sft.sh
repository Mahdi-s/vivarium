...
#SBATCH --job-name=AAM_7B_SFT

# ---------------------------------------------------------------------------
# OLMo-3 7B Think-SFT — one temperature per job (single GPU).
#
# Usage:
...
if [ "${TEMPERATURE}" = "0.6" ]; then
    SUITE="suite_7b_think_sft_temp0p6.json"
else
    SUITE="suite_7b_think_sft_temp0p0.json"
fi
...
echo "=== OLMo 7B Think-SFT: suite=${SUITE}, temperature=${TEMPERATURE} ==="
...
echo "=== OLMo 7B Think-SFT (T=${TEMPERATURE}) complete ==="
